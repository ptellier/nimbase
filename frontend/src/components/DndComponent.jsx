...
const DndComponent = () => {
  ...
  const handleDeploy = async (connectionUrl, client, server) => {
    const result = await query.updateProject(
      project._id,
      {
        ...project,
    ...
    return (
      <div ref={drop} style={{ ...style1, backgroundColor }}>
        ...
        {services.client && (
          <button
            onClick={() =>
              setServices((prevState) => ({ ...prevState, client: "" }))
            }
          >
    ...
    return (
      <div ref={drop} style={{ ...style1, backgroundColor }}>
        ...
        {services.server && (
          <button
            onClick={() =>
              setServices((prevState) => ({ ...prevState, server: "" }))
            }
          >
            <FontAwesomeIcon icon={faTrashCan} style={{ marginLeft: "10px" }} />
          </button>
  ...
  const Box = ({ name }) => {
    ...
    const opacity =
      isDragging || services.client === name || services.server === name
        ? 0.4
        : 1;
    return (
    ...
    <div style={containerStyle}>
      ...
      <Button
        ...
        isDisabled={
          ...
          project.services.length == undefined ||
          project.services.length == null
        }
        onClick={() => {
          handleDeploy(connectionUrl, services.client, services.server);
        }}
      >
        Save
      </Button>
