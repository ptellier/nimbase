...
import {faWandMagicSparkles, faQuestionCircle} from "@fortawesome/free-solid-svg-icons";
...
const ProjectEdit = () => {
  ...
  const [showHelpIconDockerfile, setShowHelpIconDockerfile] = useState(false);
  const [showHelpIconGithub, setShowHelpIconGithub] = useState(false);
  const [showHelpIconAuth, setShowHelpIconAuth] = useState(false);
  const [showHelpIconPort, setShowHelpIconPort] = useState(false);
  const [showHelpIconEnv, setShowHelpIconEnv] = useState(false);
        ...
        <div className="project-edit-container">
          ...
          <Field label="Dockerfile" type="text" name="dockerfile" value={formData.dockerfile}
            ...
            <div className="help-icon-container">
               <span
                  className={`help-icon ${showHelpIconDockerfile ? 'visible' : ''}`}
                 onMouseEnter={() => setShowHelpIconDockerfile(true)}
                  onMouseLeave={() => setShowHelpIconDockerfile(false)}
                      >
                       <FontAwesomeIcon icon={faQuestionCircle} />
                   </span>
              {showHelpIconDockerfile && (
                  <div className="help-text">Upload a valid dockerfile for your project</div>
              )}
            </div>
            </div>
          <div className="github-fields-container">
            ...
            <div className="help-icon-container">
               <span
                   className={`help-icon ${showHelpIconGithub ? 'visible' : ''}`}
                   onMouseEnter={() => setShowHelpIconGithub(true)}
                   onMouseLeave={() => setShowHelpIconGithub(false)}
               >
                       <FontAwesomeIcon icon={faQuestionCircle} />
                   </span>
              {showHelpIconGithub && (
                  <div className="help-text">Type a valid GitHuh link</div>
              )}
            </div>
            ...
            <div className="help-icon-container">
               <span
                   className={`help-icon ${showHelpIconAuth ? 'visible' : ''}`}
                   onMouseEnter={() => setShowHelpIconAuth(true)}
                   onMouseLeave={() => setShowHelpIconAuth(false)}
               >
                       <FontAwesomeIcon icon={faQuestionCircle} />
                   </span>
              {showHelpIconAuth && (
                  <div className="help-text">Input a valid GitHub auth token</div>
              )}
            </div>
          </div>
          ...
          <div className="help-icon-container">
               <span
                   className={`help-icon ${showHelpIconPort ? 'visible' : ''}`}
                   onMouseEnter={() => setShowHelpIconPort(true)}
                   onMouseLeave={() => setShowHelpIconPort(false)}
               >
                       <FontAwesomeIcon icon={faQuestionCircle} />
                   </span>
            {showHelpIconPort && (
                <div className="help-text">Input a valid entry port </div>
            )}
          </div>
          ...
          <div className="help-icon-container">
               <span
                   className={`help-icon ${showHelpIconEnv ? 'visible' : ''}`}
                   onMouseEnter={() => setShowHelpIconEnv(true)}
                   onMouseLeave={() => setShowHelpIconEnv(false)}
               >
                       <FontAwesomeIcon icon={faQuestionCircle} />
                   </span>
            {showHelpIconEnv && (
                <div className="help-text">Enter all required environment variables</div>
            )}
          </div>
