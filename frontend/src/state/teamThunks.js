...
export const createTeam = createAsyncThunk(
  ...
  async ({ teamName, description, owner, accessToken }) => {
    const response = await query.createTeam(
      teamName,
      description,
      owner,
      accessToken
...
export const removeTeamMember = createAsyncThunk(
  "team/removeMember",
  async ({ teamName, username, accessToken }) => {
    const response = await query.removeTeamMember(
      teamName,
      username,
      accessToken
    );
...
export const addTeamProject = createAsyncThunk(
  ...
  async ({ teamName, projectName, accessToken, userName }) => {
    const response = await query.addTeamProject(
      teamName,
      projectName,
      accessToken,
      userName
...
export const removeTeamProject = createAsyncThunk(
  "team/removeProject",
  async ({ teamName, projectName, accessToken, userName }) => {
    const response = await query.removeTeamProject(
      teamName,
      projectName,
      accessToken,
      userName
