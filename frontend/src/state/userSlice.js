...
import { login, logout, signup, googleLogin, refresh } from "./userThunks";
import {
  createTeam,
  deleteTeam,
  getTeam,
    ...
    builder
      ...
      .addCase(deleteTeam.fulfilled, (state, action) => {
        state.teams = state.teams.filter(
          (t) => t.teamName !== action.payload.teamName
        );
      })
      ...
      .addCase(removeTeamProject.fulfilled, (state, action) => {
        ...
        if (team) {
          team.projects = team.projects.filter(
            (p) => p !== action.payload.projectName
          );
        }
      })
      .addMatcher(
