...
export default function Maps() {
  ...
  const moveEnd = () => {
    ...
    const uniqueListing = visibleIds.filter((v, i, a) => a.indexOf(v) === i);
  ...
  useEffect(() => {
    // function to retrieve geojson from backend and dispatch it to redux store
    async function getGeojson() {
      try {
        const response = await fetch("http://localhost:8080/getlistings");
        ...
        for (let i = 0; i < data.result.length; i++) {
          structuredData.push({
            type: "Feature",
            geometry: { type: "Point", coordinates: [data.result[i].location[1], data.result[i].location[0]] },
            properties: {
